...
function retrieveProfileInfo($id) { //displaying user informations
    $conn = connect();
    ...
    $json_countrydata = json_encode($isInDatabase);
    return $json_countrydata;
}
    
//add country to database 
if(isset($_POST['mapRecordsForm'])){
    
    $country_name = $_POST["mymapsearch"];
    $id = $_SESSION['id'];
    
    //if there is a country 
        $check_name = "SELECT COUNT(*) as count FROM map_area WHERE country_name = ?";
        ...
        if ($row1["count"] > 0) { //add if there is a country 

            $check_sql = "SELECT COUNT(*) as count FROM map_records WHERE user_id = ? AND country_id = ?";
            $stmt = $conn->prepare($check_sql);
            $stmt->bind_param('is', $id, $country_name);
            $stmt->execute();
            $result = $stmt->get_result();
            $row2 = $result->fetch_assoc();
    
            if ($row2["count"] > 0) {  //if is already in database
               
                $_SESSION['status'] = "Country is already in your database";
                $_SESSION['status_code'] = "error";
                
            } else { //if not, add
                $insert_sql = "INSERT INTO map_records (user_id, country_id) VALUES (?, ?)";
                ...
                if ($stmt->execute()) {
                    ...
                    $_SESSION['status'] = "Country was added to your database";
            ...
            $_SESSION['status'] = "There is no such country";
            $_SESSION['status_code'] = "error";
        }
}
...
if(isset($_POST['countryName']))
{
    ...
    $sql ="DELETE FROM map_records WHERE user_id = $id AND country_id = '$countryName' ";

    if ($conn->query($sql) === TRUE) {
        $_SESSION['status'] = "Record deleted successfully";
        $_SESSION['status_code'] = "success";
    } else {
        $_SESSION['status'] = "Couldn't delete this record";
        $_SESSION['status_code'] = "error";
         $conn->error;
    }
        
}
    $json_countrydata = retrieveProfileInfo($id);
...
?>
            ...
            <div class="container_headers">
                ...
                <form method="post" name="mapRecordsForm" id="mapRecordsForm">
                        <input placeholder="Add..." type="text" name="mymapsearch" id="search-submit">
                        <input type="submit" name="mapRecordsForm" id="add_submit" value="+"> 
                    </span>
                </form>
                </div>
 ...
 <!--changing data mess  -->
                <div class="status">
                        ...
                        //zmienna sesyjna status istnieje i ma wartosc error
                ...
                <div id="edit-btn">
                    <a onclick="openPopup()">
                    <span class="material-symbols-sharp" id="edit">stylus</span></a>
                </div>
            ...
            <table id="stats">
                    ...
                    <tr>
                    <td>World:</td>
                    <td><?php echo $percentworld?>%</td>
                    </tr>

                    <tr>
                    <td>Europe:</td>
                    <td><?php echo $percenteuropa?>%</td>
                    </tr>
                    
                    <tr>
                    <td>Asia:</td>
                    <td><?php echo $percentasia?>%</td>
                    </tr>
                    
                    <tr>
                    <td>North America:</td>
                    <td><?php echo $percentnam?>%</td>
                    </tr>
                    
                    <tr>
                    <td>South America:</td>
                    <td><?php echo $percentsam?>%</td>
                    </tr>
                    
                    <tr>
                    <td>Australia & Oceania:</td>
                    <td><?php echo $percentaustralia?>%</td>
                    </tr>
                    
                    <tr>
                    <td>Africa:</td>
                    <td><?php echo $percentafrica?>%</td>
                    </tr> 
               
            </table>

<!--  COUNTRY LIST POPUP  -->
            <div class="countries_popup" id="countries_popup">
                    <div class="scroll-div">      
                            ...
                            <div class="scroll_countries">
                            <?php 
                                $infot = explode(",", $info);
                                foreach ($infot as $country) {
                                    $countryName = trim($country, '"[]');
                                    echo "<div class='country_popup'>";
                                    echo '<a onclick="countries_openPopup(\'' . $countryName . '\')"><span class="material-symbols-sharp">delete</span></a>';
                                    echo " ". $countryName . "</p>";
                                    echo "</div>";
                                }
                            ?>       
...
<!-- DELETING COUNTRY POPUP  -->

<div class="deleteCountry_popup" id="deleteCountry_popup">
            <header>DELETE COUNTRY</header>
                <p>Are you sure you want to delete this country from your database?</br>
              <b><span id="popupContent"></span></b></p>
                <div class="country_buttons">
                        <button name='delete_form' id="country-yes-btn" type="button" onclick="deleteCountry()">YES</button>
                        <a onclick="countries_closePopup()">
...
<svg baseprofile="tiny" fill="#9D9D9D" height="857" stroke="white" stroke-linecap="round" stroke-linejoin="round" stroke-width=".2" version="1.2" viewbox="0 0 2000 857" width="2000" xmlns="http://www.w3.org/2000/svg">
...
<!-- //green if in database  -->
<script type="text/javascript">
    var jsonCountryData = <?php echo $json_countrydata; ?>;
    for (var i = 0; i < jsonCountryData.length; i++) {
    var country_id = jsonCountryData[i];
}
        document.querySelectorAll(".allPaths").forEach(e => {
            //if is in database
        if (jsonCountryData.includes(e.id)) {
        e.style.fill = "#5E9350"; // green
    } else {
        e.style.fill = "#9D9D9D"; // default 
        }
